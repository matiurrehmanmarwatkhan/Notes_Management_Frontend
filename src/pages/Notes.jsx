    ...
    <div className="max-w-7xl mx-auto px-6 w-full">
      <nav className="flex justify-between items-center py-5 border-b border-white/10 mb-10">
        ...
        <div className="flex gap-5 items-center">
          <button
            className="bg-transparent text-slate-100 border border-white/10 hover:bg-white/5 px-5 py-2.5 rounded-lg font-semibold transition-all duration-300"
            onClick={logout}
          >
            Logout
          </button>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          className="bg-primary text-white hover:bg-primary-hover hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(99,102,241,0.3)] px-5 py-2.5 rounded-lg font-semibold transition-all duration-300"
          onClick={() => openModal()}
        >
          + Add Note
        </button>
      ...
      {loading ? (
        <div className="text-center mt-12 text-slate-400 text-lg">
          Loading notes...
        </div>
      ) : notes.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-8">
          {notes.map((note) => (
            <div
              key={note._id}
              className="p-6 rounded-xl bg-card-bg border border-white/10 hover:-translate-y-1 hover:shadow-2xl hover:border-white/20 transition-all flex flex-col animate-fade-in"
            >
              <div className="flex-grow">
                ...
                <p className="text-slate-400 mb-5 whitespace-pre-wrap">
                  {note.description}
                </p>
              </div>
              <div className="flex gap-3 justify-end mt-auto">
                <button
                  className="bg-transparent text-slate-100 border border-white/10 hover:bg-white/5 px-3 py-1.5 rounded-lg font-medium text-sm transition-all duration-300"
                  onClick={() => openModal(note)}
                >
                  Edit
                </button>
                <button
                  className="bg-danger text-white hover:bg-danger-hover hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(239,68,68,0.3)] px-3 py-1.5 rounded-lg font-medium text-sm transition-all duration-300"
